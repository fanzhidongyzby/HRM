...
<%
	...
	docList = docDao.findAll(staffId);
...
&gt;&nbsp;<%=s.getStaffName() %>&nbsp;
[<a href="staffDetail.jsp?staffId=<%=s.getStaffID() %>">个人信息</a>&nbsp;&gt;&nbsp;员工档案]
...
<%
 ...
 <%if(hasValid){ %>
		...
		onClick="javascript:location.href='docEdit.jsp?staffId=<%=staffId %>&opp=add'"><%} %>
		  </td>
		 </TR>
        <TR>
...
<%Iterator<document> it =docList.iterator();
            while(it.hasNext()){
               oneDoc = (document)it.next();
               String abs=oneDoc.getDocAbs();
               if(abs==null)
               	abs="";
               String com=oneDoc.getDocCom();
               if(com==null)
               	com="";
               %>
		</tr>	
			<tr height="35" align="center" style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
			<td><%=oneDoc.getDocNum() %></td>
			<td><%=oneDoc.getDocName() %></td>
			<td><%=abs %></td>
			<td><%=com %></td>
			<td ><%=oneDoc.getDocType() %></td><td>
<input name="checkDoc" type="checkbox" <%if(!hasValid){ %>disabled<%} %> value="<%=oneDoc.getDocID() %>"/>
			<input name="editStaff" type="button" value="编辑" <%if(!hasValid){ %>disabled<%} %>
	onClick="javascript:location.href='docEdit.jsp?staffId=<%=staffId %>&opp=edit&docId=<%=oneDoc.getDocID() %>'"/>
			</td>
		</tr>		
			<%} %>
		  </TABLE><br>
	...
	<INPUT onclick="selectAll(this.checked)"; type=checkbox name=checkAll <%if(!hasValid){ %>disabled<%} %> value="<%=oneDoc.getDocID() %>">全选
...
function deleteSum()
{
	var checkList=document.getElementsByName("checkDoc");	
	var idList="";
	var k=0;
	...
	if(confirm("将要删除当前用户的"+k+"条档案，确定这么做吗？"))
	{
		window.location.href='../ActionServlet?actionPath=document&opp=delete&staffId=<%=staffId%>&args='+idList;
	}
}
</script>
